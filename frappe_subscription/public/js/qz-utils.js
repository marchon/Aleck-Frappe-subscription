 ...
 * Displays an frappe.msgprint if not ready.
 */
function notReady() {
    ...
    // If a printer hasn't been selected, display a message.
        ...
        frappe.msgprint('Please select a printer first by using the "Detect Printer" button.');
        return true;
    }
    return false;
}
...
function qzDonePrinting() {
    // Alert error, if any
    if (qz.getException()) {
        frappe.msgprint('Error printing:\n\n\t' + qz.getException().getLocalizedMessage());
        qz.clearException();
        return;
    }

    // Alert success message
    frappe.msgprint('Successfully sent print data to "' + qz.getPrinter() + '" queue.');
}
...
function useDefaultPrinter() {
    if (isLoaded()) {
        ...
        window['qzDoneFinding'] = function() {
            ...
            frappe.msgprint(printer !== null ? 'Default printer found: "' + printer + '"':
 ...
 *    window['qzDoneFinding'] = function() { frappe.msgprint(qz.getPrinters()); };
 ***************************************************************************/
...
function appendEPCL(data) {
    if (data == null || data.length == 0) {
        return frappe.msgprint('Empty EPCL data, skipping!');
    }
 ...
 * Usage:
 *    qz.findNetworkInfo();
 *    window['qzDoneFindingNetwork'] = function() {frappe.msgprint(qz.getMac() + ',' +
	*       qz.getIP()); };
 ***************************************************************************/
function listNetworkInfo() {
    if (isLoaded()) {
        ...
        window['qzDoneFindingNetwork'] = function() {
            frappe.msgprint("Primary adapter found: " + qz.getMac() + ", IP: " + qz.getIP());
...
function useAlternatePrinting() {
    if (isLoaded()) {
        ...
        frappe.msgprint('Alternate CUPS printing set to "' + !alternate + '"');
    }
}
 ...
 * /dist/ folder.
 *    qz.findPorts();
 *    window['qzDoneFindingPorts'] = function() { frappe.msgprint(qz.getPorts()); };
 ***************************************************************************/
 ...
 *    window['qzDoneOpeningPort'] = function(port) { frappe.msgprint(port); };
 ***************************************************************************/
function openSerialPort() {
    if (isLoaded()) {
        ...
        window['qzDoneOpeningPort'] = function(portName) {
            if (qz.getException()) {
                frappe.msgprint("Could not open port [" + portName + "] \n\t" +
                        qz.getException().getLocalizedMessage());
                qz.clearException();
            } else {
                frappe.msgprint("Port [" + portName +  "] is open!");
            }
        };
    }
}
 ...
 *    window['qzDoneClosingPort'] = function(port) { frappe.msgprint(port); };
 ***************************************************************************/
function closeSerialPort() {
    if (isLoaded()) {
        ...
        window['qzDoneClosingPort'] = function(portName) {
            if (qz.getException()) {
                frappe.msgprint("Could not close port [" + portName + "] \n\t" +
                        qz.getException().getLocalizedMessage());
                qz.clearException();
            } else {
                frappe.msgprint("Port [" + portName +  "] closed!");
            }
        };
    }
}
...
function sendSerialData() {
    if (isLoaded()) {
        ...
        window['qzSerialReturned'] = function(portName, data) {
            if (qz.getException()) {
                frappe.msgprint("Could not send data:\n\t" + qz.getException().getLocalizedMessage());
                qz.clearException();
            } else {
                if (data == null || data == "") {       // Test for blank data
                    frappe.msgprint("No data was returned.")
                } else if (data.indexOf("?") !=-1) {    // Test for bad data
                    frappe.msgprint("Device not ready.  Please wait.")
                } else {                                // Display good data
                    frappe.msgprint("Port [" + portName + "] returned data:\n\t" + data);
                }
            }
        };
    }
